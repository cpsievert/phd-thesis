...
and \ref{fig:eechidna-2p-2}, electorates may be quieried via directly
...
proposed (Wilkinson \protect\hyperlink{ref-Wilkinson:2005}{2005});
(Wilkinson and Wills \protect\hyperlink{ref-Wilkinson:2008}{2008});
(Dang and Wilkinson \protect\hyperlink{ref-Wilkinson:2012}{2012}).
Probably the most universally useful scagnostic is the outlying measure
which helps identify projections of the data space that contain outlying
...
Although projects mentioned earlier in this thesis (e.g.
\textbf{pitchRx}, \textbf{XML2R}, and \textbf{LDAvis}) could use more
mork, I consider them as completed projects, especially from an academic
publishing standpoint. The more interesting work to be done involves the
projects I've been contributing to over the past 2-3 years --
\textbf{animint} and \textbf{plotly}. Through this work, I've learned
that mixed computation is hard, but it helps tremendously to have
collaborators that complement each others strengths. Furthermore, making
such a system practically useful requires a \emph{huge} amount of work
that is not directly rewarded with academic publications. In order for
such a project to be worthwhile academically (at least within the field
of statistical graphics), there needs to a separation of concerns
between lower-level rendering details and more novel/experimental
things. The \textbf{animint} project is too ambitious in this sense in
that it takes responsibility over everything. On the other hand, the
\textbf{plotly} project builds upon the plotly.js project, allowing it
to leverage the hard work of a thriving open source community and a team
of full-time software engineers.
...
potential projects related to \textbf{plotly} that would be interesting
...
\item
  ...
  \textbf{ggplot2} extension packages. A particularly intriguing example
  would be the \textbf{GGally} package (Schloerke et al.
  \protect\hyperlink{ref-GGally}{2016}).
\item
  Apply and/or build upon the existing infrastructure in \textbf{plotly}
  to address domain-specific visualization problem(s). This could be
  anything from the
\item
  Extend the functionality of \textbf{plotly} itself
\end{enumerate}
...
\setcounter{enumi}{1}
\item
  Extend the functionality of other packges
\item
\item
  Retrieve the state of an interactive visualization. The
  \texttt{highlight()} function does provide a
\end{enumerate}

Along these lines, as eluded to in the
\protect\hyperlink{limitations}{limitations} section of
\protect\hyperlink{linking-views-without-shiny}{linking views without
shiny}, there are a number of things \textbf{plotly} could do to expand
its support for linked views. Aside from those software development
issues,

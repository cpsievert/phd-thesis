...
within or even between two arbitrary interactive graphics systems. North
...
can program reactive web application entirely within R (Chang et al.
...
which design a new R interface for creating web graphics.
...
animations and linked views with relatively small amount of effort
required by those familiar with \textbf{ggplot2}. This extension is
...
showSelected}. In the case of \textbf{plotly}, animations and linked
views (even between non-plotly graphics) are supported as well as a much
more rich taxnomomy of interactions. These features are discussed at
length in \href{https://cpsievert.github.io/plotly_book/}{plotly for R}.
...
in how far we can take it. This is largely because you lose a lot of
information about the raw data, and any high-level semantics tied to the
visual mappings.

many of the semantics tied Assuming the goal is to have a general
purpose interface for highly dynamic interactive web graphics from R,

we're better off building a new interface designed exactly for this
purpose. The first serious attempt in this direction was the R package
\textbf{rCharts}, whose R interface is heavily inspired by
\textbf{lattice} (Vaidyanathan \protect\hyperlink{ref-rCharts}{2013}).
The most impressive result of \textbf{rCharts}'s design is its ability
to interface with many different JavaScript charting libraries. However,
\textbf{rCharts} has little to no support for coordination of dynamic
linked views from R.
...
\subsection{The MVC Architecture}\label{the-mvc-architecture}

The Model-View-Controller (MVC) paradigm is a popular programming
technique for designing user interfaces with a minimal amount of
dependencies, and therefore, maximizing responsiveness. Responsiveness
is absolutely crucial for interactive graphics since it greatly impacts
on our ability to make observations, draw generalizations and generate
hypotheses (Z. L. A. J. Heer
\protect\hyperlink{ref-2014-latency}{2014}).
...
downloads for the last year amongst the leading R packages for
...
\caption{\label{fig:cranlogs}CRAN downloads over the past year from
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr::}\KeywordTok{include_graphics}\NormalTok{(}\StringTok{"images/github-stars"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
htmlwidgets framework and sort widgets by the number of GitHub stars.}
\end{figure}

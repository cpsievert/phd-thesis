...
(\protect\hyperlink{ref-model-vis-paper}{2015}) suggest to plot the
model in the data space and use dynamic interactive statistical graphics
to do so. Interactive graphics have also proved to be useful for
exploratory model analysis, a situation where we have many models to
evaluate, compare, and critique (Unwin, Volinsky, and Winkler
...
files in Table \ref{tab:pitchfx}. In fact, \textbf{XML2R} has also been
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tmp[}\KeywordTok{grep}\NormalTok{(}\StringTok{"game//inning//atbat"}\NormalTok{, }\KeywordTok{names}\NormalTok{(tmp))][}\DecValTok{2}\NormalTok{]}
...
\ref{tab:pitchfx}. In fact, any combination of these files can be
...
\StringTok{  }\CommentTok{# Select variables for covariates in probabilistic models}
...
\CommentTok{# Here is a way to create better facet titles w/o changing data values}
...
\KeywordTok{strikeFX}\NormalTok{(}
  \NormalTok{strikes, }\DataTypeTok{geom =} \StringTok{"raster"}\NormalTok{, }
...
\KeywordTok{strikeFX}\NormalTok{(}
  \NormalTok{decisions, }\DataTypeTok{geom =} \StringTok{"raster"}\NormalTok{, }
  \DataTypeTok{density1 =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{des =} \StringTok{"Called Strike"}\NormalTok{), }
  \DataTypeTok{density2 =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{des =} \StringTok{"Ball"}\NormalTok{), }
  \DataTypeTok{layer =} \KeywordTok{facet_grid}\NormalTok{(. ~}\StringTok{ }\NormalTok{stand, }\DataTypeTok{labeller =} \NormalTok{relabel)}
\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{strikeFX}\NormalTok{(}
  \NormalTok{decisions, }\DataTypeTok{model =} \NormalTok{m, }
  \DataTypeTok{layer =} \KeywordTok{facet_grid}\NormalTok{(. ~}\StringTok{ }\NormalTok{stand, }\DataTypeTok{labeller =} \NormalTok{relabel)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{strikeFX}\NormalTok{(}
  \NormalTok{decisions, }\DataTypeTok{model =} \NormalTok{m, }
  \DataTypeTok{density1 =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{inning_side =} \StringTok{"top"}\NormalTok{), }
  \DataTypeTok{density2 =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{inning_side =} \StringTok{"bottom"}\NormalTok{),}
  \DataTypeTok{layer =} \KeywordTok{facet_grid}\NormalTok{(p_throws ~}\StringTok{ }\NormalTok{stand, }\DataTypeTok{labeller =} \NormalTok{relabel2)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
(\protect\hyperlink{ref-Cook:2007uk}{1996}): finding Gestalt, posing
queries, and making comparisons. Furthermore, as Figure
\ref{fig:pedestrians-cog-tour} shows, and Wickham, Cook, and Hofmann
(\protect\hyperlink{ref-model-vis-paper}{2015}) writes, these same
interactive techniques can also be a helpful for understanding,
inspecting, and diagnosing statistics models. The next case study on
Zika virus infections demonstrates how interactive graphics can be
useful for tracking disease outbreak and detecting data quality issues.
...
reason for the negative counts (McNeil and Victor
...
\begin{figure}
\centering
\includegraphics{screenshots/houston}
...
\StringTok{  }\KeywordTok{add_fun}\NormalTok{(function(plot) \{}
  ...
  \NormalTok{\}) %>%}
\StringTok{  }\KeywordTok{add_fun}\NormalTok{(function(plot) \{}
    \NormalTok{plot %>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(city ==}\StringTok{ "San Antonio"}\NormalTok{) %>%}\StringTok{ }
...
\begin{figure}
\centering
\includegraphics{screenshots/houston-vs-sa}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(diamonds, }\KeywordTok{aes}\NormalTok{(cut, }\DataTypeTok{fill =} \NormalTok{clarity)) +}
\StringTok{  }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{position =} \StringTok{"fill"}\NormalTok{)}

\KeywordTok{ggplotly}\NormalTok{(p, }\DataTypeTok{originalData =} \OtherTok{FALSE}\NormalTok{) %>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{ydiff =} \NormalTok{ymax -}\StringTok{ }\NormalTok{ymin) %>%}\StringTok{ }
\StringTok{  }\KeywordTok{add_text}\NormalTok{(}
    \DataTypeTok{x =} \NormalTok{~x, }\DataTypeTok{y =} \NormalTok{~}\DecValTok{1} \NormalTok{-}\StringTok{ }\NormalTok{(ymin +}\StringTok{ }\NormalTok{ymax) /}\StringTok{ }\DecValTok{2}\NormalTok{,}
    \DataTypeTok{text =} \NormalTok{~}\KeywordTok{ifelse}\NormalTok{(ydiff >}\StringTok{ }\FloatTok{0.02}\NormalTok{, }\KeywordTok{round}\NormalTok{(ydiff, }\DecValTok{2}\NormalTok{), }\StringTok{""}\NormalTok{),}
...
\begin{figure}
\centering
\includegraphics{screenshots/StatBin}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(MASS::geyser, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{waiting, }\DataTypeTok{y =} \NormalTok{duration)) +}
\StringTok{  }\KeywordTok{geom_density2d}\NormalTok{()}

\KeywordTok{ggplotly}\NormalTok{(p, }\DataTypeTok{originalData =} \OtherTok{FALSE}\NormalTok{) %>%}\StringTok{ }
\StringTok{  }\KeywordTok{group_by}\NormalTok{(piece) %>%}
\StringTok{  }\KeywordTok{slice}\NormalTok{(}\KeywordTok{which.min}\NormalTok{(y)) %>%}\StringTok{ }
\StringTok{  }\KeywordTok{add_text}\NormalTok{(}
    \DataTypeTok{text =} \NormalTok{~level, }\DataTypeTok{size =} \KeywordTok{I}\NormalTok{(}\DecValTok{9}\NormalTok{), }\DataTypeTok{color =} \KeywordTok{I}\NormalTok{(}\StringTok{"black"}\NormalTok{), }\DataTypeTok{hoverinfo =} \StringTok{"none"}
  \NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{screenshots/StatDensity2d}
...
\KeywordTok{subplot}\NormalTok{(}
  ...
  \KeywordTok{plot_ly}\NormalTok{(mpg, }\DataTypeTok{x =} \NormalTok{~cty, }\DataTypeTok{y =} \NormalTok{~hwy) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{add_markers}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.2}\NormalTok{, }\DataTypeTok{name =} \StringTok{"alpha"}\NormalTok{),}
  \KeywordTok{plot_ly}\NormalTok{(mpg, }\DataTypeTok{x =} \NormalTok{~cty, }\DataTypeTok{y =} \NormalTok{~hwy) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{add_markers}\NormalTok{(}\DataTypeTok{symbol =} \KeywordTok{I}\NormalTok{(}\DecValTok{1}\NormalTok{), }\DataTypeTok{name =} \StringTok{"hollow"}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{col2 <-}\StringTok{ }\NormalTok{viridisLite::}\KeywordTok{inferno}\NormalTok{(}\DecValTok{10}\NormalTok{)}
...
\KeywordTok{subplot}\NormalTok{(}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~cyl, }\DataTypeTok{colors =} \NormalTok{col1) %>%}
\StringTok{    }\KeywordTok{colorbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"ggplot2 default"}\NormalTok{),}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~cyl, }\DataTypeTok{colors =} \NormalTok{col2) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{colorbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Inferno"}\NormalTok{),}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~cyl, }\DataTypeTok{colors =} \NormalTok{col3) %>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{col1 <-}\StringTok{ "Pastel1"}
...
\KeywordTok{subplot}\NormalTok{(}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~}\KeywordTok{factor}\NormalTok{(cyl), }\DataTypeTok{colors =} \NormalTok{col1),}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~}\KeywordTok{factor}\NormalTok{(cyl), }\DataTypeTok{colors =} \NormalTok{col2),}
  \KeywordTok{add_markers}\NormalTok{(p, }\DataTypeTok{color =} \NormalTok{~}\KeywordTok{factor}\NormalTok{(cyl), }\DataTypeTok{colors =} \NormalTok{col3)}
\NormalTok{) %>%}\StringTok{ }\KeywordTok{hide_legend}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{screenshots/sizes}
\caption{\label{fig:sizes}Controlling the size range via \texttt{sizes}
(measured in pixels).}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(Sepal.Length~Sepal.Width*Petal.Length*Petal.Width, }\DataTypeTok{data =} \NormalTok{iris)}
\CommentTok{# to order categories sensibly arrange by estimate then coerce factor }
...
via \texttt{semi\_join()}. As Figure \ref{fig:linetypes} demonstrates,
once we have the data is filtered, mapping city to \texttt{color} or
\texttt{linetype} is trivial. The color palette can be altered via the
\texttt{colors} argument, and follows the same rules as
\protect\hyperlink{scatterplots}{scatterplots}. The linetype palette can
be altered via the \texttt{linetypes} argument, and accepts R's
...
\begin{figure}
\centering
\includegraphics{screenshots/linetypes}
...
Team \protect\hyperlink{ref-RCore}{2016}). By simply providing a
...
\StringTok{    }\KeywordTok{colorbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"default"}\NormalTok{) %>%}
...
\StringTok{    }\KeywordTok{colorbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"zsmooth"}\NormalTok{) %>%}
...
\StringTok{    }\KeywordTok{colorbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"nbins"}\NormalTok{) %>%}
...
\begin{figure}
\centering
\includegraphics{screenshots/simple}
...
\NormalTok{plots <-}\StringTok{ }\KeywordTok{lapply}\NormalTok{(vars, function(var) \{}
  \KeywordTok{plot_ly}\NormalTok{(economics, }\DataTypeTok{x =} \NormalTok{~date, }\DataTypeTok{y =} \KeywordTok{as.formula}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\StringTok{"~"}\NormalTok{, var))) %>%}
...
\begin{figure}
\centering
\includegraphics{screenshots/economics}
...
\begin{figure}
\centering
\includegraphics{screenshots/prepopulate}
...
\begin{figure}
\centering
\includegraphics{screenshots/joint}
...
\ref{fig:trelliscope} demonstrates, the \textbf{trelliscopejs} package
provides lower-level tools that allow one to create trelliscope displays
without \textbf{shiny} (Hafen and Schloerke
\protect\hyperlink{ref-trelliscopejs}{2016}).
...
\StringTok{  }\KeywordTok{xlim}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{37}\NormalTok{) +}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\DecValTok{9}\NormalTok{, }\DecValTok{47}\NormalTok{) +}\StringTok{ }\KeywordTok{theme_bw}\NormalTok{() +}
\StringTok{  }\KeywordTok{facet_trelliscope}\NormalTok{(}
    \NormalTok{~}\StringTok{ }\NormalTok{manufacturer +}\StringTok{ }\NormalTok{class, }\DataTypeTok{nrow =} \DecValTok{2}\NormalTok{, }\DataTypeTok{ncol =} \DecValTok{4}\NormalTok{, }
    \DataTypeTok{as_plotly =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{plotly_args =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{dynamicTicks =} \NormalTok{T)}
  \NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Group name is used to populate a title for the dropdown}
\NormalTok{sd <-}\StringTok{ }\NormalTok{SharedData$}\KeywordTok{new}\NormalTok{(txhousing, ~city, }\DataTypeTok{group =} \StringTok{"Choose a city"}\NormalTok{)}
...
\StringTok{  }\KeywordTok{add_bars}\NormalTok{(}\DataTypeTok{x =} \NormalTok{~has, }\DataTypeTok{y =} \NormalTok{~}\KeywordTok{factor}\NormalTok{(city, }\DataTypeTok{levels =} \NormalTok{city), }
           \DataTypeTok{hoverinfo =} \StringTok{"none"}\NormalTok{) %>%}
...
\KeywordTok{subplot}\NormalTok{(}
  \KeywordTok{plot_ly}\NormalTok{(d, }\DataTypeTok{y =} \NormalTok{~disp, }\DataTypeTok{color =} \KeywordTok{I}\NormalTok{(}\StringTok{"black"}\NormalTok{)) %>%}\StringTok{ }
\StringTok{    }\KeywordTok{add_boxplot}\NormalTok{(}\DataTypeTok{name =} \StringTok{"overall"}\NormalTok{),}
...
\hypertarget{nested-selections}{\subsubsection{Nested
selections}\label{nested-selections}}
...
\CommentTok{# ggplotly() automatically adds rownames as a key if none is provided}
...
\NormalTok{gg <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(gapminder, }\KeywordTok{aes}\NormalTok{(gdpPercap, lifeExp, }\DataTypeTok{color =} \NormalTok{continent)) +}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{size =} \NormalTok{pop, }\DataTypeTok{frame =} \NormalTok{year, }\DataTypeTok{ids =} \NormalTok{country)) +}
...
\StringTok{  }\KeywordTok{plot_ly}\NormalTok{(}\DataTypeTok{x =} \NormalTok{~gdpPercap, }\DataTypeTok{y =} \NormalTok{~lifeExp, }\DataTypeTok{size =} \NormalTok{~pop, }
          \DataTypeTok{text =} \NormalTok{~country, }\DataTypeTok{hoverinfo =} \StringTok{"text"}\NormalTok{) %>%}
...
\NormalTok{gapminder$continent <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(}
  \NormalTok{gapminder$continent, }\DataTypeTok{levels =} \KeywordTok{names}\NormalTok{(}\KeywordTok{sort}\NormalTok{(meanLife))}
\NormalTok{)}
...
\NormalTok{countryByArea <-}\StringTok{ }\KeywordTok{read.table}\NormalTok{(}
  \StringTok{"https://bit.ly/2h6vscu"}\NormalTok{,}
...
\NormalTok{search <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}
  \StringTok{"http://google.com/#q="}\NormalTok{, curl::}\KeywordTok{curl_escape}\NormalTok{(}\KeywordTok{rownames}\NormalTok{(mtcars))}
\NormalTok{)}
...
\begin{figure}
\centering
\includegraphics{screenshots/click-open}
...
CRAN mirror download logs. When I became the maintainer and
\href{https://github.com/ropensci/plotly/graphs/contributors}{sole
contributor} of the project over a year ago, several other R packages
seemed to be more popular, but as Figure \ref{fig:cranlogs} suggests,
\textbf{plotly} is now the most widely used, and by a significant
amount. The recent spike in CRAN downloads was due to a
\href{http://moderndata.plot.ly/upgrading-to-plotly-4-0-and-above/}{major
update} in \textbf{plotly} which introduced a lot of new features, but
the new linked views and animation framework has yet to even hit CRAN.
Once this framework becomes more stable and available on CRAN, I am
confident popularity will keep increasing.
...
As shown in Figure \ref{fig:github-stars}, \textbf{plotly} also owns the
most GitHub stars among all R packages built on the \textbf{htmlwidgets}
framework. Github stars provide a mechanism for users of open-source
software to indicate their interest in a project on the worlds largest
repository of software. This metric is even being used in academic
studies on the popularity of open-source software projects (Hudson
Borges \protect\hyperlink{ref-stars}{2016}).
...
htmlwidgets framework and sort widgets by the number of GitHub stars. As
of writing, plotly has 791 stars which is the most among all
htmlwidgets.}
\end{figure}

\subsection{LDAvis}\label{ldavis}

The R package \textbf{LDAvis} currently has
\href{https://web.archive.org/web/20161122210119/https://github.com/cpsievert/LDAvis}{226
github stars} and inspired a python port which currently has
\href{https://web.archive.org/web/20161122210642/https://github.com/bmabey/pyLDAvis}{369
github stars}. The software was also used to generate a winning
submission to
\href{http://web.archive.org/web/20140215133100/http://streaming.stat.iastate.edu/~dicook/video-competition/}{the
2014 statistical graphics video competition} and is now featured in the
\href{http://stat-graphics.org/movies/}{ASA's video library}.

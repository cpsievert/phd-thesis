...
This chapter focuses on linking multiple views for posing queries,
making comparisons, and exploring relationships in multi-dimensional
data. Three different case studies are used to demonstrate these
techniques

\section{Case Studies}\label{case-studies}

\subsection{Exploring pedestrian
counts}\label{exploring-pedestrian-counts}

The first example uses pedestrian counts from around the city published
on the City of Melbourne's open data platform (Melbourne, n.d.). The
...
\textbf{pedestrians} (Sievert, n.d.). The main dataset of interest is
...
Figure \ref{fig:missing} can not tell us is \emph{where} these missings
actually occur. To investigate this question, it would be helpful to
link this information to actual time series.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(ggplot2)}
\NormalTok{is_na <-}\StringTok{ }\KeywordTok{with}\NormalTok{(pedestrians, }\KeywordTok{tapply}\NormalTok{(Counts, }\DataTypeTok{INDEX =} \NormalTok{Name, function(x) }\KeywordTok{sum}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(x))))}
\NormalTok{pedestrians$Name <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(pedestrians$Name, }\KeywordTok{names}\NormalTok{(}\KeywordTok{sort}\NormalTok{(is_na)))}
\KeywordTok{ggplot}\NormalTok{(pedestrians, }\KeywordTok{aes}\NormalTok{(Name, }\DataTypeTok{fill =} \KeywordTok{is.na}\NormalTok{(Counts))) +}\StringTok{ }
\StringTok{  }\KeywordTok{geom_bar}\NormalTok{() +}\StringTok{ }\KeywordTok{coord_flip}\NormalTok{() +}\StringTok{ }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{y =} \OtherTok{NULL}\NormalTok{) +}
\StringTok{  }\KeywordTok{scale_fill_discrete}\NormalTok{(}\StringTok{"Is missing"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
missingness and time via downsampling techniques. Figure
\ref{fig:missing-by-time} displays an interactive version of Figure
\ref{fig:missing} linked to a downsampled (stratisfied within sensor
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr::}\KeywordTok{include_graphics}\NormalTok{(}\StringTok{"images/pedestrians-missing"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
components. Partly due to to its widespread availability in R, the
...
\(\hat{Var(T_t)}\)) (Hyndman, Wang, and Laptev, n.d.); (Wang, n.d.).
From these estimates, they produce other informative summary statistics,
such as the seasonal peak (\(Max_{t}(\hat{S_t})\)), trough
(\(Min_{t}(\hat{S_t})\)), spike ( \(Var[(Y_t - \bar{Y})^2]\)); as well
as trend linearity and curvature (coefficients from a 2nd degree
polynomial fit to the estimated trend \(\hat{T_t}\)).

After computing these 7 different summary statistics for each sensor, we
can graphically explore the 7-dimensional space that they live in, as
...
interpolates between random 2D projections (aka, a grand tour) of the 7
dimensions, and on the right-hand side is a parallel coordinates plot
...
standard deviation 1. This reveals an unusual station (Tin Alley-Swanson
St) in terms of both curvature and linearity.

The top left panel shows a grand tour of the 7 dimensional space and the
top right panel a parallel coordinates plot.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr::}\KeywordTok{include_graphics}\NormalTok{(}\StringTok{"images/pedestrians-stl-tour"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:pedestrians-stl-tour}Linking views of the raw data
(second row) with the summary statistics of seasonal trend decomposition
(first row). The top left panel shows a grand tour of the 7 dimensional
space and the top right panel a parallel coordinates plot. In both
views, it is apparent that one station (Tin Alley-Swanson St),
highlighted in red, has irregular curvature and linearity compared to
the other stations. By linking to (sampled) raw counts and the hourly
IQR, we can see that this station experiences relatively low traffic
(red) compared to the average (black). See
\href{https://vimeo.com/189037958}{here} for the corresponding video and
\href{http://cpsievert.github.io/pedestrians/stl-tour/}{here} for the
interactive figure.}
\end{figure}

From these 7 summary statistics, we could explore the 7 dimensional
space constructed

Missingness is just one type of feature we may explore and use to guide
our analysis.

The package also includes geographic information about each sensor in
the \texttt{sensors} data set and time-series cognostics for

\subsubsection{All together now}\label{all-together-now}
...
Earo Wang for sparking my interest in the pedestrians data, helping
...
\caption{\label{fig:pipeline}A diagram of the pipeline between the data and
graphics.}
\end{figure}
...
\CommentTok{#> Error: GeomXspline was built with an incompatible version of ggproto.}
\CommentTok{#> Please reinstall the package that provides this extension.}
...
each attribute that \textbf{plotly} leverages internally. At print time,
it ensures each attribute is of the correct type, and also searches for
any unsupported attributes that may have been specified by the user (and
throws a warning that the attribute will be ignored).
...
Hyndman, Rob J, Earo Wang, and Nikolay Laptev. n.d. \emph{Anomalous:
...
Melbourne, City of. n.d. ``City of Melbourne's Open Data Platform.''
...
---------. n.d. \emph{Pedestrians: Tools for Exploring Melbourne's
...
Wang, Earo. n.d. \emph{Tscognostics: Time Series Cognostics}.

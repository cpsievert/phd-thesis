...
shows, the model can reside in several locations.

For relatively simple interactions, like identification (scenario A) and
resizing (scenario B), the model should reside within the web browser.
For more complicated interactions, the model may have to be written in R
by the user. In theory, this round-trip back to R should only be
necessary if the model requires information that is not already
contained in the JSON object. In practice, the browser-based model
doesn't always cover all of the user's use cases, so it is always a good
idea to allow other models to subscribe to the controller.

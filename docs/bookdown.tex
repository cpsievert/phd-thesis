...
\protect\hyperlink{ref-pedestrians}{2016}). The main dataset of interest
is named \texttt{pedestrians} and contains nearly 1 million counts, but
...
from this selection set.

\begin{figure}
\centering
\includegraphics{images/pedestrians-cog-tour-acf}
...
blue). See \href{}{here} for the corresponding video and
\href{http://cpsievert.github.io/pedestrians/cog-tour/}{here} for the
interactive figure.}
\end{figure}
...
groups, which we could verify and inspect further via hierarchical
clustering.
...
green) confirms our prior suspicion from earlier -- sensors on Swanson
St (high autocorrelation) are most different from sensors on the
outskirts of the city (low autocorrelation). Increasing the number of
clusters to 3-4 splits off the unusual sensors that we identified in out
previous analysis (Waterfront City, Birrarung Marr, and Tin
Alley-Swanson St).
...
---------. 2016. \emph{Pedestrians: Tools for Exploring Melbourne's

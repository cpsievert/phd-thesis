...
A hybrid MVC does not have to rely on __shiny__, in fact, it can be used link multiple views without requiring a full redraw or R code to be re-executed. Furthermore, it can be implemented in such a way that is arguably easier to express from R compared to a full-blown __shiny__ app. Figure \@ref(fig:scenarios) gives a high-level overview of how this works in the __plotly__ package. Scenario B shows a situation where a brush event manipulates a model defined in a __shiny__ app (via the `event_data()` function) which requires a full redraw. Scenario A shows a situation where a brush event directly manipulates a linked view via a __plotly__'s hybrid MVC model; and in this case, the JavaScript function `Plotly.restyle()` is used to simply highlight the selected points in both views.
...
```{r highlight-bars, echo = FALSE, fig.cap = "Brushing a scatterplot to "}
knitr::include_graphics("images/highlight-boxplot")
```
...
* R developers that have authored a custom __ggplot2__ geom and want to enable `ggplotly()` to translate their geom to plotly.
* R users that know some JavaScript and want to enable custom features that plotly.js does not natively support.
...
```{r hover-log, fig.cap = "Using `onRender()` to attach a JavaScript callback to a 'plotly_click' event that opens a google search."}
library(plotly)
library(htmlwidgets)

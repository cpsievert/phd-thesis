...
A hybrid MVC doesn't have to rely on __shiny__, in fact, it can be used link multiple views without requiring a full redraw or R code to be re-executed. Furthermore, it can be implemented in such a way that is arguably easier to express from R compared to a full-blown __shiny__ app. Figure \@ref(fig:scenarios) gives a high-level overview of how this works in the __plotly__ package. Scenario B shows a situation where a brush event manipulates a model defined in a __shiny__ app (via the `event_data()` function) which requires a full redraw. Scenario A shows a situatuin where a brush event manipulates a __plotly__'s custom model for linking views which, in this case, uses the JavaScript function `Plotly.restyle()` to simply highlight the selected points in both views.
...
```{r hybrid-pipeline, echo = FALSE, fig.cap = "The GGobi pipeline in comparison to a hybrid MVC paradigm"}
...
```{r ggpairs, fig.asp = 1, fig.width = 8, fig.cap = "An interactive version of the generalized pairs plot made via the `ggpairs()` function from the **GGally** package"}
...
```{r many-traces, fig.cap = "Median house sales with one trace per city."}
...
In [Bars & histograms](#bars-histograms), we leveraged a number of algorithms in R for computing the "optimal" number of bins for a histogram, via `hist()`, and routing those results to `add_bars()`.
...
```{r maps, fig.width = 8, fig.cap = "Three different ways to render a map. On the top left is plotly's default cartesian coordinate system, on the top right is plotly's custom geographic layout, and on the bottom is mapbox."}
...
```{r ggmosaic, error = TRUE, fig.cap = "Using ggmosaic and ggplotly() to create advanced interactive visualizations of categorical data"}
library(ggmosaic)
p <- ggplot(data = cc) +
...
```{r multiple-htmlwidgets, fig.cap = "Printing multiple htmlwidget objects with `tagList()`. To render tag lists at the command line, wrap them in `browsable()` "}
library(htmltools)
library(plotly)
...
```{r flexbox, fig.cap = "Arranging multiple htmlwidgets with flexbox"}
tags$div(
...
```{r fluid, fig.cap = "Arranging multiple htmlwidgets with `fluidPage()` from the **shiny** package."}
library(shiny)
fluidPage(
  fluidRow(p),
  fluidRow(
...
```{r, echo = FALSE}
knitr::include_graphics("images/proportions.png")
```
...
```{r, echo = FALSE}
knitr::include_graphics("images/subplot.png")
```
...
```{r ggplot2-subplots, fig.cap = "Arranging multiple faceted ggplot2 plots into a plotly subplot."}
...
```{r trelliscope, eval = FALSE, fig.cap = "Using plotly within a trelliscope"}
...
<!--chapter:end:../plotly_book/internals.Rmd-->
...
<!--chapter:end:../plotly_book/extensions.Rmd-->

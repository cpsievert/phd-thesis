---
title: "Leveraging Data on the Web for Statistical Analysis"
...
The World Wide Web brought about exciting new opportunities to share information  and data with anyone. "Open Data" is important, especially for the scientific community, where verified conclusions maintain integrity and enable progress of the discipline. Although researchers are generally willing to share their data, the movement has not been widely adopted largely due to a lack of "systems that make it quick and easy to share data" [@Tenopir:2011fl] [@Pampel:2013a]. Fortunately, there are a number of efforts to reduce this burden and make data repositories on the Web more visible [@Pampel:2013bi] [@Dillon:2013].

<!-- A bit on socialised data analysis? -->

Unfortunately, freely available data on the Web is often difficult to acquire and/or embedded within unstructured documents, making it difficult to incorporate into a data analysis workflow. 
...
1. A high level interface with direct access to curated data. In this case, the interface is typically restricted to a single data source, but users can obtain tidy data [@tidy-data] without any knowledge of protocols or Web Technologies. Some interfaces, such as **pitchRx** [@Sievert:2014], perform all the steps of data curation under the hood and require a tremendous amount of work by the author. Other interfaces may simply wrap an existing web API for accessing already tidy data. 
